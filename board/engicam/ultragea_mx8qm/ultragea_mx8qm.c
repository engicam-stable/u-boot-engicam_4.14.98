...
static iomux_cfg_t pad_gpio[] = {
	SC_P_QSPI1A_DATA3 |  MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CTRL),//GPIO4_IO23
	SC_P_QSPI1A_DATA2 |  MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CTRL),//GPIO4_IO24
	SC_P_QSPI1A_DATA1 |  MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CTRL),//GPIO4_IO25
	SC_P_USB_SS3_TC0  |  MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CTRL),//GPIO4_IO03
	SC_P_MIPI_CSI0_GPIO0_01 |  MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CTRL),//eth0 reset
	SC_P_MIPI_CSI0_I2C0_SDA |  MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CTRL),//eth1 reset
};

static void setup_iomux_gpio(void)
{
	imx8_iomux_setup_multiple_pads(pad_gpio, ARRAY_SIZE(pad_gpio));
}

#define BB_GPIO_3V3_1 IMX_GPIO_NR(4, 25)
#define BB_GPIO_3V3_2 IMX_GPIO_NR(4, 24)
#define BB_GPIO_3V3_3 IMX_GPIO_NR(4, 23)
#define BB_GPIO_3V3_4 IMX_GPIO_NR(4, 3)
#define ETH0_RESET IMX_GPIO_NR(1, 28)
#define ETH1_RESET IMX_GPIO_NR(1, 26)

static void board_gpio_init(void)
{
	puts("gpio: init power\n");
	setup_iomux_gpio();
	
	/* eth phy reset*/
	gpio_request(ETH0_RESET, "eth_phy0_rst");
	gpio_direction_output(ETH0_RESET, 1);
	gpio_request(ETH1_RESET, "eth_phy1_rst");
	gpio_direction_output(ETH1_RESET, 1);
	...
	gpio_request(BB_GPIO_3V3_4, "bb_3v3_4");
	gpio_direction_output(BB_GPIO_3V3_4, 1);
...
int board_late_init(void)
{
	...
	env_set("board_name", "ULTRAGEA");
	env_set("board_rev", "MX8QM");
#endif
	...
	if (fdt_file && !strcmp(fdt_file, "undefined")) {
		if (m4_boot)
			env_set("fdt_file", "ultragea-imx8qm-rpmsg.dtb");
		else
			env_set("fdt_file", "ultragea-imx8qm.dtb");
	}
